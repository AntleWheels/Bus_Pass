...
def add_buspass():
    if request.method == "POST":
        ...
        if not all([bus_no, department, roll_number, admission_year, boarding_point, booking_date, validity_date, amount, student_image, student_signature, incharge_signature, principal_signature, college_seal]):
            flash("Please fill all the fields")
        ...
        new_buspass = models.Bus_pass(
            ...
            id_no=current_user.id_no  # Assuming `current_user` has an `id_no` attribute
        )
